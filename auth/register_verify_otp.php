...
if (isset($_POST['verify_now'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM users WHERE email = '$email' AND verification_code = '$user_otp'");

    if (mysqli_num_rows($result) > 0) {
        mysqli_query($conn, "UPDATE users SET verification_code = NULL WHERE email = '$email'");
        unset($_SESSION['temp_email']);
        echo "<script>alert('Account Activated!'); window.location='../index1.php';</script>";
    } else {
...
<body>
    <form method="POST" class="otp-card" autocomplete="one-time-code">
        ...
        <button type="submit" name="verify_now" class="btn-activate">Activate Account</button>
    </form>
</body>
</html>